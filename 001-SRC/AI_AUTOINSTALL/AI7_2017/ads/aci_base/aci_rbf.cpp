...
struct resbuf* rbnewitem(rbufdesc_t* desclst, int restype)
{
	...
	if((p = (struct resbuf*) ads_malloc(sizeof(struct resbuf))) == NULL)
		return NULL;
...
struct resbuf* rbnewitem(rbufdesc_t* desclst, int restype, TCHAR* resval)
{
	struct resbuf* p;
	if((p = rbnewitem(desclst, restype)) == NULL)
		return NULL;
	if(((p->resval).rstring = (TCHAR*) ads_calloc(wcslen(resval) + 1, sizeof(TCHAR))) == NULL) {
		rbdelitem(desclst, p);
		return NULL;
	}
	wcscpy((p->resval).rstring, resval);
	return p;
}
...
struct resbufdb* rbnewitem(rbufdbdesc_t* desclst, int restype)
{
	...
	if((p = (struct resbufdb*) ads_malloc(sizeof(struct resbufdb))) == NULL)
		return NULL;
...
struct resbufdb* rbnewitem(rbufdbdesc_t* desclst, int restype, TCHAR* resval)
{
	struct resbufdb* p;
	if((p = rbnewitem(desclst, restype)) == NULL)
		return NULL;
...
struct resbuf* rbdelitem(rbufdesc_t* desclst, resbuf* itemptr)
{
	...
	if((itemptr->restype == RTSTR) && ((itemptr->resval).rstring != NULL))
		ads_free((itemptr->resval).rstring);
...
// rbassoc(): funcao que retorna a sublista que contem a chave indicada
// desclst - descritor da lista
// dxfcode - codigo dxf do item da lista
struct resbuf* rbassoc(struct resbuf* fstitem, short dxfcode)
{
	struct resbuf* p = fstitem;

	while(p != NULL)
	{
		if(p->restype == dxfcode)
			return p;
		p = p->rbnext;
	}
