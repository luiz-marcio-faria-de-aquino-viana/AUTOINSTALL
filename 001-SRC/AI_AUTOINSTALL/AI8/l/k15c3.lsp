...
;;                  pt1 pt2 pta ptb nfile file deltx delty)
  ...
  (initget "Yes No")
  (setq plfile (getkword (strcat "\nImprimir em um arquivo <" #plfile ">: ")))
  (if plfile (setq #plfile plfile))
  ...
  (if (null fform) (setq fform (cons "A0 - ESPECIAL" (list deltx delty))) )
  ...
  (write-line (strcat "PLOTER " nfile " " (substr (car fform) 1 2) " " #plfile ) file)
