...
(defun C:MkDET(/ prj nlr arq txt cnt pt1 pt2 pti dcr ndt)
  (setvar "cmdecho" 0)
  (initget 1 "EL ES H G TE TI IE AR")
  (setq
    prj (getkword "\nDetalhe EL/ES/H/G/TE/TI/IE/AR: ")
    nlr (strcat prj "-detalhe")
    arq (open (strcat "/ai-det/" prj "/acad.blk") "r")
    txt "Thru"
    cnt 0
  );endsetq
  ...
  (command
    ...
    "change" "p" "" "p" "la" nlr ""
    "wblock" (strcat "/ai-det/" prj "/" ndt) "" pti "p" ""
    "oops"
    "scale" "p" "" pti (#SCL)
  );endcommand
    (setvar "blipmode" 1)
    (setvar "highlight" 1)
  (setq
    arq (open (strcat "/ai-det/" prj "/acad.blk") "a")
  );endsetq
  (write-line
    (strcat ndt " - " dcr) arq
  );endwrite
