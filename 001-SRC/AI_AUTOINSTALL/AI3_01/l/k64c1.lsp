...
(defun joinpline (ss1 ss2 / enm1 enm2 p1i p1f p2i p2f pts1 pts2 pt pti ptf)
  ...
  (setq
    p1i (trans (cdr (assoc 10 (entget (entnext           enm1)))) 1 0)
    p1f (trans (cdr (assoc 10 (entget (entnext (entnext enm1))))) 1 0)
    p2i (trans (cdr (assoc 10 (entget (entnext           enm2)))) 1 0)
    p2f (trans (cdr (assoc 10 (entget (entnext (entnext enm2))))) 1 0)
  ) ; end setq 
  (if (setq pt (inters p1i p1f p2i p2f nil))
    (progn
      ...
      (if (or (< (car (trans pt 0 1))                      0)
              (< (car (trans pt 0 1)) (car (trans pts1 0 1))) )
      ...
      (if (or (< (car (trans pt 0 1))                      0)
              (< (car (trans pt 0 1)) (car (trans pts2 0 1))) )
    ...
    (if (and (= (cdr (assoc 0 (entget (car ss1)))) "LINE")  (= (cdr (assoc 0 (entget (car ss2)))) "LINE"))
      ...
      (if (and (= (cdr (assoc 0 (entget (car ss1)))) "LINE")  (= (cdr (assoc 0 (entget (car ss2)))) "POLYLINE"))
        ...
        (if (and (= (cdr (assoc 0 (entget (car ss1)))) "POLYLINE") (= (cdr (assoc 0 (entget (car ss2)))) "LINE"))
          (lntopl ss2 ss1)
          (prompt "\n* ERROR * Selecao de entidades invalidas.")
        ); end if 
      );end if
    ); end if
  ); end if
  (command ".ucs" "r" "$line")
  (setvar "ucsicon" oldico)
  (command ".undo" "e")
