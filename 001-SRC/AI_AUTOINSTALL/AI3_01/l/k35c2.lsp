...
(defun eqdc_dsquadro(pti qdr lscar lsdim / HTT HTC HTD HLT HLC1 HLC HLD HLS WCL NLE NTE NCE TTAB lt ll nc nt nl tc pt0
  numq potq it lt lc1 lc2 cir1 pot1 vf1 bt1 prot1 fas1 lt1 tip1 num1 it1 potc1 numc1 potdq vfq btq bpq protq fasq oldblip oldecho)
...
;; c:eqdc(): rotina de desenho do quadro de cargas com dimensionamento simplificado
(defun c:eqdc(/ qdr vfase btmin pti fred temp flg lscar)
  ...
  (if (setq lscar (eqdc_lvquadro qdr))
    (eqdc_dsquadro pti qdr lscar (eqdc_dmquadro #VFASE (atof #BTMIN) #TEMP fred lscar))
  ) ; end if
