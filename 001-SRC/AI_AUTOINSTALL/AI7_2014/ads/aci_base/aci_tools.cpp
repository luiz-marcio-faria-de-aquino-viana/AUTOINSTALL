...
int tools_explodeattr(ads_name ss)
{
	...
	struct resbuf oldlay;
	long len;

	str_t str;
	int rscode;

	ads_getvar(_T("CLAYER"), &oldlay);

	acedCommandS(RTSTR, _T(".ucs"), RTSTR, _T(""), RTNONE);
	...
	for(long i = 0; i < len; i++)
	{
		...
		if (rscode == RTNORM) {
			...
			if(dxf_enttype != NULL) {
				if(wcscmp(dxf_enttype->resval.rstring, _T("INSERT")) == 0) {
	...
	acedCommandS(RTSTR, _T(".layer"), RTSTR, _T("s"), RTSTR, oldlay.resval.rstring, RTSTR, _T(""), RTNONE);
	acedCommandS(RTSTR, _T(".redraw"), RTNONE);
...
{
	struct resbuf *ent;
	struct resbuf *dxf_currlayer;

	ads_name curr_enm;
	ads_name next_enm;
	struct resbuf *next_ent;

	struct resbuf *dxf_enttype;
	struct resbuf *dxf_text;
	struct resbuf *dxf_layer;
	struct resbuf *dxf_point;
	struct resbuf *dxf_endpoint;
	struct resbuf *dxf_height;
	struct resbuf *dxf_rot;
	struct resbuf *dxf_align;
	...
	acedCommandS(RTSTR, _T(".layer"), RTSTR, _T("s"), RTSTR, dxf_currlayer->resval.rstring, RTSTR, _T(""), RTNONE);
	...
	while( !seqend && (ads_entnext(curr_enm, next_enm) == RTNORM) ) {
		...
		if(dxf_enttype != NULL)
		{
			if(wcscmp(dxf_enttype->resval.rstring, _T("SEQEND")) == 0) {
				seqend = true;
			}
			else {
				...
				if( (dxf_point != NULL) &&
					...
					(dxf_align != NULL) )
				{
					if(wcslen(dxf_text->resval.rstring) > 0)
					{
						if(dxf_align->resval.rint == 1) //Center
						{
							acedCommandS(RTSTR, ".text", RTSTR, "c", RT3DPOINT, dxf_endpoint->resval.rpoint, RTREAL, dxf_height->resval.rreal, RTREAL, dxf_rot->resval.rreal, RTSTR, dxf_text->resval.rstring, RTNONE);
						}
						else if(dxf_align->resval.rint == 2) //Right
						{
							acedCommandS(RTSTR, ".text", RTSTR, "r", RT3DPOINT, dxf_endpoint->resval.rpoint, RTREAL, dxf_height->resval.rreal, RTREAL, dxf_rot->resval.rreal, RTSTR, dxf_text->resval.rstring, RTNONE);
						}
						else //Left
						{
							acedCommandS(RTSTR, ".text", RT3DPOINT, dxf_point->resval.rpoint, RTREAL, dxf_height->resval.rreal, RTREAL, dxf_rot->resval.rreal, RTSTR, dxf_text->resval.rstring, RTNONE);
						}
					}
				}
			}
...
int tools_explodeattr_explodeblock(ads_name enm)
{
	...
	if(dxf_layer != NULL) {
		acedCommandS(RTSTR, _T(".explode"), RTENAME, enm, RTNONE);
		...
		while(ads_entnext(curr_enm, next_enm) == RTNORM) {
			...
			if(dxf_enttype != NULL)
			{
				if(wcscmp(dxf_enttype->resval.rstring, _T("ATTDEF")) == 0) {
					ads_entdel(next_enm);
				}
				else {
					old_dxf_layer = rbassoc(next_ent, (short)8);
					if(old_dxf_layer != NULL) {
						ads_free(old_dxf_layer->resval.rstring);
						old_dxf_layer->resval.rstring = (TCHAR*)ads_calloc(wcslen(dxf_layer->resval.rstring) + 1, sizeof(TCHAR));
						wcscpy(old_dxf_layer->resval.rstring, dxf_layer->resval.rstring);
						ads_entmod(next_ent);
					}
				}
			}
